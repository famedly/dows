...
use translate::{WsToDnsState, close_frame, dns_to_ws, ws_to_dns};
...
async fn handle_connection(mut sock: TcpStream, policy: Arc<Policy>) -> io::Result<()> {
	let Some(upstream_addr) = websocket_handshake(&mut sock, &policy).await? else {
		return Ok(());
	};
...
/// Performs the HTTP phase of a connection: reads and parses the request,
/// answers the info page and rejected or malformed requests directly, and
/// completes the WebSocket upgrade otherwise.
///
/// Returns the requested upstream address once the `101 Switching
/// Protocols` response has been sent, or `None` if the request was already
/// answered and the connection should be closed.
async fn websocket_handshake(sock: &mut TcpStream, policy: &Policy) -> io::Result<Option<String>> {
	let mut buf = [0; 65536];
	let mut data_len = 0;
	...
	if policy.allowed_origins.as_ref().is_some_and(|allowed| {
		request.headers.iter().any(|h| {
			h.name.eq_ignore_ascii_case("origin")
				&& !allowed.split(",").map(str::trim).any(|x| x.as_bytes() == h.value)
		})
	}) {
	...
	Ok(Some(parse_upstream_addr(&percent_decode_str(final_segment).decode_utf8_lossy())))
}
...
async fn forward_requests(
	...
	// When ws_to_dns stops because the dns output buffer is full, the next
	// iteration needs to rerun ws_to_dns without reading new data.
	let mut read = true;

	let code = loop {
		if read {
			let size @ 1.. = sock_r.read(&mut buf.ws[buf.ws_remainder.end..]).await? else {
				break 1000;
			};
			buf.ws_remainder.end += size;
		}
		let result = ws_to_dns(&mut buf);
		if buf.pong > 0
			&& let Some(sock_w) = sock_w.lock().await.as_mut()
		{
			sock_w.write_all(&buf.ws[..buf.pong]).await?;
		}
		if buf.dns_out > 0 && up_w.write_all(&buf.dns[..buf.dns_out]).await.is_err() {
			break 1000;
		}
		if let Err(code) = result {
			break code;
		}
		read = buf.ws_remainder.is_empty() || (buf.dns_out == 0 && buf.pong == 0);
		// Remove the pongs and DNS messages sent above from the buffers.
		buf.ws.copy_within(buf.ws_remainder.clone(), 0);
		buf.ws_remainder = 0..buf.ws_remainder.len();
		buf.pong = 0;
		buf.dns.copy_within(buf.dns_out..buf.dns_pos, 0);
		buf.dns_pos -= buf.dns_out;
		buf.dns_out = 0;
	};

	if let Some(mut sock_w) = sock_w.lock().await.take() {
		let close_msg = close_frame(buf.dns.first_chunk_mut().unwrap(), code, "");
		sock_w.write_all(close_msg).await?;
	}
