...
async fn handle_connection(mut sock: TcpStream) -> io::Result<()> {
	let mut buf = [0; 65536];
	let mut data_len = 0;
...
/// already specify one.
fn parse_upstream_addr(final_segment: &str) -> String {
	let s = percent_decode_str(final_segment).decode_utf8_lossy();
