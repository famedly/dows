...
//! 2-byte big-endian length prefix used by DNS over TCP (RFC 1035, Section
//! 4.2.2), so the test server implements just that framing rather than pulling
//! in a full DNS stack.
...
impl Harness {
	async fn start() -> Self {
		...
		let mut child = Command::new(env!("CARGO_BIN_EXE_dows"))
			.env("DOWS_ALLOWED_UPSTREAMS", "*")
			.env("DOWS_ALLOWED_ORIGINS", "*")
			.arg("[::1]:0")
			.stdin(Stdio::null())
			.stdout(Stdio::piped())
			.spawn()
		...
		let endpoint = loop {
			...
			assert!(!line.is_empty(), "dows should print listener address before closing stdout");
...
#[tokio::test]
async fn ping_and_close_in_single_syscall() {
	let harness = Harness::start().await;

	// Hand-rolled handshake and frames: a WebSocket client library can't
	// guarantee that both frames leave in one syscall, but a single write of
	// both frames over loopback arrives at the proxy in one read, so the
	// translation sees the close frame while a pong is still pending.
	let mut sock = TcpStream::connect(&harness.proxy_endpoint).await.expect("connect to proxy");
	let request = format!(
		"GET /{} HTTP/1.1\r\n\
         Host: localhost\r\n\
         Upgrade: websocket\r\n\
	...
	sock.write_all(request.as_bytes()).await.expect("send handshake");

	// Read the 101 response up to the end of the header section.
	let mut response = Vec::new();
	let mut byte = [0];
	while !response.ends_with(b"\r\n\r\n") {
		sock.read_exact(&mut byte).await.expect("read handshake response");
		response.push(byte[0]);
	}
	assert!(response.starts_with(b"HTTP/1.1 101 "), "unexpected response: {response:?}");

	// A masked ping (payload DE AD BE EF, mask 0F) directly followed by a
	// masked close frame (code 1000, zero mask), sent with a single syscall.
	let frames: [u8; 18] = [
		0x89, 0x84, 0x0F, 0x0F, 0x0F, 0x0F, 0xD1, 0xA2, 0xB1, 0xE0, // ping
		0x88, 0x82, 0, 0, 0, 0, 0x03, 0xE8, // close
	];
	sock.write_all(&frames).await.expect("send ping and close");

	// The proxy must answer the ping before completing the closing handshake.
	let mut reply = Vec::new();
	sock.read_to_end(&mut reply).await.expect("read pong and close");
	assert_eq!(
		reply,
		[
			0x8A, 0x04, 0xDE, 0xAD, 0xBE, 0xEF, // pong with unmasked payload
			0x88, 0x02, 0x03, 0xE8, // close, code 1000
		]
	);
}
